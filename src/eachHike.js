...
async function populateReviews() {
  ...
  try {
    ...
    querySnapshot.forEach((docSnap) => {
      ...
      reviewCard.querySelector(".level").textContent = `${level}`;
      reviewCard.querySelector(".season").textContent = `${season}`;
      reviewCard.querySelector(".scrambled").textContent = `${scrambled}`;
      reviewCard.querySelector(".flooded").textContent = `${flooded}`;
      ...
      for (let i = 0; i < safeRating; i++) {
        starRating += '<span class="material-icons text-warning">star</span>';
      }
...
/**
 * starRating += '<span class="material-icons text-warning">star</span>';
starRating += '<span class="material-icons text-secondary">star_outline</span>';
 */
